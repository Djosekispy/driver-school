...
const UpdateTrafficSignScreen = () => {
  const router = useRouter();
  const { id } = useLocalSearchParams();
  const { getSignById, updateSign } = useFirebase();
  ...
  // Carrega os dados do sinal existente
  useEffect(() => {
    const loadSignData = async () => {
      try {
        setIsFetching(true);
        const sign = await getSignById(id as string);
        ...
        if (sign) {
          setFormData({
            ...
            rules: sign.rules.length > 0 ? sign.rules : [''],
            commonMistakes: sign.commonMistakes.length > 0 ? sign.commonMistakes : [''],
            imageUrl: sign.imageUrl
          });
          setLocalImage(sign.imageUrl);
        }
      } catch (error) {
        Alert.alert('Erro', 'Não foi possível carregar os dados do sinal');
        router.back();
      } finally {
        setIsFetching(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (localImage && localImage !== formData.imageUrl && !localImage.startsWith('http')) {
        await uploadImageToFirebase(localImage);
      }

      await updateSign(id as string, {
        name: formData.name,
      ...
      Alert.alert('Sucesso', 'Sinal de trânsito atualizado com sucesso!', [
        { text: 'OK', onPress: () => router.back() }
      ]);

    } catch (error: any) {
      console.error('Erro ao atualizar sinal:', error);
      Alert.alert('Erro', error.message || 'Não foi possível atualizar o sinal');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <ScrollView 
          ...
          showsVerticalScrollIndicator={false}
        >
          {/* Campos do formulário (iguais à tela de adição) */}
          {/* Campo: Nome */}
          <View className="mb-4">
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                <Text className="text-white font-semibold ml-2">Atualizar Sinal</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
...
export default UpdateTrafficSignScreen;